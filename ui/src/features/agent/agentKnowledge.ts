...
- Use review_series_attempts with confirm=true after the user explicitly chooses takes. selected_latest addresses human-visible shot_numbers; all_latest is only valid for choosing pending takes and mirrors “Use pending takes”. Rejecting is deliberately limited to one shot at a time. Supply attempt_id only when the user chose a particular historical attempt; otherwise the executor resolves the latest eligible completed attempt.
